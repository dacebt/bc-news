...
- BCN-008 is not complete until its separately authorized production-hostname, secret, database-backend, and recovery observations pass.
...
3. **An operator can verify that the deployable configuration has no alternate public Worker origin, no stored credential, no unintended data route, and a recoverable production database.** — Repository-side exposure, secret, dependency, and D1 evidence comes first; deployed hostname and recovery proof complete the capability only under separate outward-action authorization.

## Order rationale

The composed acceptance spine is already green in the isolated worktree. The operator control plane comes first because it guards side effects and cost; the reader boundary then proves the public product survives that split; production exposure and recovery assurance consume both contracts and require the final authorized Cloudflare observation.
