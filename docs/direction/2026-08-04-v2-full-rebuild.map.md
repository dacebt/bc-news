...
3. **Evidence integrity** — the functional core enforces the evidence-date window, rejects duplicate evidence ids before lossy transforms, and deterministically orders the remaining messages, whose ids are unique within that evidence input, by `(ts, id)`; nullable `author_name` is exercised by the canonical evidence corpus. Closes the skeleton review's core follow-ups.
