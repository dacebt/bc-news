...
**Update reason:** real model evaluation harness — separate model behavior from harness integrity and product verification.
...
| Model evaluation | Observation and retention of model behavior on declared inputs, including behavior production rejects. | Acceptance gate |
...
| Subject outcome | The typed behavior observed from the model, such as completed, rejected, or infrastructure-incomplete. | Harness outcome |
...
|---|---|---|
| Production generation | Hard-failing creation and publication of one edition through four production model steps. | A model rejection fails generation; it is not evaluation evidence here. |
| Model evaluation | Durable observation of real model behavior across declared benchmark runs and trials. | Rejection is a subject outcome; lost evidence is a harness failure. |
