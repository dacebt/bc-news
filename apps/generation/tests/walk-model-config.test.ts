...
it("forces recorded generation adapters ahead of live developer configuration", () => {
	const developerModelConfig = JSON.stringify({
		...
		announcements: { adapter: "lmstudio", model: "local", sampling: { temperature: 1, top_p: 0.95, top_k: 20 }, reasoning_effort: "none" },
		packaging: { adapter: "lmstudio", model: "local", sampling: { temperature: 1, top_p: 0.95, top_k: 20 }, reasoning_effort: "none" },
	});
