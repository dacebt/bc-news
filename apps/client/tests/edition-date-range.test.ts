...
const RANGE: EditionDateRange = { minDate: EARLIEST_PUBLICATION_DATE, maxDate: "2026-07-25" };
...
describe("maxServableDate", () => {
	// The suite is pinned to America/New_York (UTC-4 in July), so 22:00 local
	// is already the next UTC day. A local-clock read would answer the day
	// before.
	it("returns the utc date when local date differs", () => {
		expect(maxServableDate(new Date(Date.UTC(2026, 6, 25, 2, 0, 0)))).toBe("2026-07-25");
	});
...
describe("stepWithinRange bounds", () => {
	it("answers null stepping past the latest served date", () => {
		expect(stepWithinRange("2026-07-25", 1, RANGE)).toBeNull();
	});
...
describe("isServableDate", () => {
	it("accepts the bounds themselves", () => {
		...
		expect(isServableDate("2026-07-25", RANGE)).toBe(true);
	});

	it("rejects the day before the earliest served date", () => {
		expect(isServableDate("2026-01-14", RANGE)).toBe(false);
	});

	it("rejects the day after the latest served date", () => {
		expect(isServableDate("2026-07-26", RANGE)).toBe(false);
	});
	...
	// The bounds alone would accept this: '2026-01-15' < '2026-05' <
	// '2026-07-25' as strings. Only the shape check refuses it.
	it("rejects a truncated date that sorts between the bounds", () => {
		expect(isServableDate("2026-05", RANGE)).toBe(false);
	});
