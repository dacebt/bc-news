...
test("rejects repository reference traversal and malformed commits", () => {
...
test("current output identities retain semantic hashes but no source-file hashes", () => {
...
test("current declaration and artifact reject recursive byte ownership fields", () => {
	...
	expect(EvaluationScorecardArtifactSchema.safeParse({ version: 2, id: "scorecard-one", source_payloads: {} }).success).toBe(false);
