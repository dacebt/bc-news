...
test("accepts an ordered baseline and subject roster of local-data references", () => {
	const declaration = { version: 3, id: "series-input", scorecards: [
...
test("rejects recursive byte ownership at the current artifact boundary", () => {
	expect(EvaluationLongitudinalScorecardArtifactSchema.safeParse({ version: 3, id: "series-one", source_payloads: {}, declaration_sha256: "1".repeat(64) }).success).toBe(false);
});

test("keeps the historical longitudinal v1 and v2 contracts explicit", () => {
