...
import { evaluationConfigIdentity } from "../src/evaluation-artifact";
...
test("summarizes products retained by final-product rejection", async () => {
	...
	expect(result.benchmark.version).toBe(4);
...
test("keeps LM Studio sampling configuration and posture in comparison context", async () => {
	...
	const explicit = clone(V4BenchmarkRunSchema.parse(result.benchmark));
