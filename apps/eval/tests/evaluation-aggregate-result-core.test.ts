...
import { EvaluationScorecardArtifactSchema, type EvaluationScorecardArtifact } from "../src/evaluation-scorecard";
...
function scorecard(step: EvaluationScorecardArtifact["scorecards"][number]["production_step"]) {
	const adapterByStep = {
		...
		announcements_copyedit: { adapter: "cloudflare_ai_gateway" as const, gateway: { selection: "named" as const, id: "private-gateway" }, model: "@cf/meta/llama", temperature: 0.1 },
	} as const;
	return {
		production_step: step,
		adapter: adapterByStep[step],
		scorecard_context: unknownContext(),
		sample_counts: {
...
function aggregateSource(): EvaluationScorecardArtifact {
	return EvaluationScorecardArtifactSchema.parse({
		version: 2,
		id: "scorecard-one",
		created_at: "2026-08-17T12:00:00.000Z",
		source_reference: { repository: "bc-news", commit_sha: COMMIT, path: "apps/eval/summaries/scorecard-one.json" },
		...
		configuration: {
			...
			exact_config: {
				production_steps: {
					...
					announcements_copyedit: { adapter: "cloudflare_ai_gateway", gateway: { selection: "named", id: "private-gateway" }, model: "@cf/meta/llama", temperature: 0.1 },
				},
			},
		},
		repetition_count: 1,
		sources: {
			corpus_manifest_path: "apps/eval/local-data/manifest.json",
			benchmark_runs: [{
				ordinal: 1,
				corpus_fixture_id: "fixture-one",
				benchmark_run_id: "benchmark-one",
				path: "apps/eval/local-data/benchmark-one.json",
			...
			annotations: {
				path: "apps/eval/local-data/annotations.json",
			...
			qualitative_reviews: {
				path: "apps/eval/local-data/reviews.json",
...
test("builds a strict aggregate projection without private evidence fields", () => {
	...
	expect(aggregate).toMatchObject({
		...
		source_scorecard: { version: 2, id: "scorecard-one" },
		cohort: { id: "corpus-one", fixture_count: 6, repetition_count: 1, raw_message_count: 385, prepared_message_count: 149 },
	...
	expect(EvaluationAggregateResultSchema.safeParse({
		...aggregate,
		source_reference: { repository: "bc-news", commit_sha: COMMIT, path: "private.json" },
	}).success).toBe(false);
});
...
test("rejects unsupported source scorecard versions", () => {
	expect(() => buildEvaluationAggregateResult(
		{ version: 1, id: "historical-scorecard", created_at: "2026-08-17T12:00:00.000Z" } as unknown as EvaluationScorecardArtifact,
...
test("formats neutral reports and comparisons", () => {
	const left = buildEvaluationAggregateResult(aggregateSource(), {
		id: "aggregate-left",
		createdAt: "2026-08-17T12:30:00.000Z",
		cohortId: "corpus-one",
	});
	const changedSource = aggregateSource();
	const changedRole = changedSource.scorecards[1];
	expect(changedRole).toBeDefined();
	if (changedRole === undefined) throw new Error("Expected main_story_copyedit scorecard");
	changedRole.adapter = { adapter: "lmstudio", model: "lmstudio-model-v2", reasoning_effort: "provider_default" };
	const right = buildEvaluationAggregateResult(changedSource, {
		id: "aggregate-right",
	...
	expect(report).toContain("Evaluation aggregate result v1: aggregate-left");
	...
	expect(comparison.differences).toContain("aggregate.roles[1].subject.model");
	...
	expect(comparisonText).toContain("aggregate.roles[1].subject.model");
