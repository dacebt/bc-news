...
export async function verifyEvaluationScorecards(temporaryRoot?: string): Promise<string> {
	...
	try {
		...
		const artifact = buildEvaluationScorecard(input, {
			id: "controlled-evaluation-scorecard-v3",
			createdAt: controlled.createdAt,
		});
		const serialized = json(artifact);
		for (const forbidden of ["source_payloads", "base64", "declaration_sha256", "benchmark_run_sha256", "reference_sha256"]) {
			assertProof(
				!serialized.includes(forbidden),
				`Current scorecard retained forbidden byte ownership field ${forbidden}`,
			);
		}
		assertProof(
			artifact.version === 3
		...
		assertProof(
			artifact.scorecards.every(({ production_step, scorecard_context }) => {
				if (scorecard_context.state !== "identified") {
					return true;
				}
		...
		assertProof(
			artifact.scorecards.every(({ scorecard_context }) => (
				scorecard_context.state === "identified"
					&& JSON.stringify(scorecard_context.projection.corpus_source_reference)
						=== JSON.stringify(input.corpus.sourceReference)
			)),
			"Scorecard semantic context omitted the local corpus source reference",
		);
		...
		try {
			await loadEvaluationScorecardInput(controlledDeclarationSourcePath, localDataRoot);
		} catch {
			tamperRejected = true;
		}
		...
		const restoredArtifact = buildEvaluationScorecard(
			...
			{ id: artifact.id, createdAt: restored.createdAt },
		);
		...
		try {
			await loadEvaluationScorecardArtifact(artifact.id, results, { localDataRoot });
		} catch {
		...
		assertProof(
			(await loadEvaluationScorecardArtifact(artifact.id, results, { localDataRoot: movedRoot })).id === artifact.id,
			"Relocated local-data root did not reopen the scorecard",
		);
		...
		let v2LocalManifestRejected = false;
		try {
			await loadEvaluationScorecardInput(restoredDeclarationSourcePath, localDataRoot);
		} catch {
			v2LocalManifestRejected = true;
		}
...
async function main(): Promise<void> {
	process.stdout.write(`${await verifyEvaluationScorecards()}\n`);
}
