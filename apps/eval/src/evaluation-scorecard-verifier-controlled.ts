...
function controlledSourceProvenance(codeCommit: string) {
...
async function buildControlledRuns(repositoryRoot: string, manifestPath: string, evidenceRoot: string, options: { codeCommit: string; gateway: boolean }): Promise<{ runs: ControlledRun[]; createdAt: string; annotations: JsonObject; reviews: JsonObject }> {
	...
	const resultsDirectory = join(evidenceRoot, "benchmark-runs");
	const configPath = join(evidenceRoot, "benchmark.config.json");
		...
		for (const entry of corpus.entries) {
			...
			if (options.gateway) {
				...
				const result = await evaluateBenchmarkCommand({
					fixturePath: join(repositoryRoot, entry.evidencePath),
					configPath,
					resultsDirectory,
...
export async function buildControlledEvaluationScorecardInputV2(repositoryRoot: string, manifestPath: string, options: { directory: string; codeCommit: string }): Promise<{ declarationPath: string; createdAt: string }> {
	...
	const annotationPath = join(evidenceRoot, "annotations.json");
	const reviewPath = join(evidenceRoot, "reviews.json");
	...
	await writeFile(declarationPath, json({
		...
		runs: runs.map(({ run, path, entry }, index) => ({
			ordinal: index + 1,
		...
		annotations: {
			path: repositoryPath(repositoryRoot, annotationPath),
			bundle_id: annotations.id as string,
		},
		qualitative_reviews: {
			path: repositoryPath(repositoryRoot, reviewPath),
...
async function rebaseControlledScorecardDeclaration(
	...
	const sourceManifest = EvaluationReferenceManifestV3Schema.parse(JSON.parse(await readFile(rebasedManifestPath, "utf8")) as unknown);
	await writeFile(rebasedManifestPath, json({
		...sourceManifest,
		selection: await sourceReferenceForLocalFile(targetLocalDataRoot, rebasePath(sourceManifest.selection.path)),
...
export async function buildControlledLocalScorecard(
	...
	const controlled = await buildControlledEvaluationScorecardInputV3(repositoryRoot, manifestPath, sourceLocalDataRoot, { codeCommit });
	const sourceDeclarationPath = await rebaseControlledScorecardDeclaration(sourceLocalDataRoot, localDataRoot, declarationDirectory);
