...
const MainStoryJudgeScoresSchema = z.strictObject({
...
export interface JudgeStepResult {
	readonly scores: z.infer<typeof MainStoryJudgeScoresSchema>;
...
function parseJudgeOutput(
	...
	rawText: string,
): { scores: z.infer<typeof MainStoryJudgeScoresSchema>; reasoning: string } {
	...
	return { scores: MainStoryJudgeScoresSchema.parse(parsed.data.scores), reasoning: parsed.data.reasoning };
}
