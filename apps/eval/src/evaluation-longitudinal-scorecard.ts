...
export {
	...
	StableLongitudinalContextV2Schema,
...
const LocalPathSchema = Trimmed.refine((value) => {
	if (value.startsWith("/") || value.includes("\\")) return false;
	const normalized = normalize(value).replaceAll("\\", "/");
	if (normalized !== value) return false;
	const parsed = posix.normalize(value);
	if (parsed === "." || parsed !== value) return false;
	return value.split("/").every((segment) => segment.length > 0 && segment !== "." && segment !== "..");
}, "Path must be a normalized contained POSIX path");

export const LocalSourceReferenceSchema = z.strictObject({
...
const StableProjectionSchema = z.strictObject({
	...
	ordered_gateway_requests: z.array(z.strictObject({
		observation_ordinal: z.number().int().positive(),
...
const SourceIdentitySchema = z.strictObject({ ordinal: z.number().int().positive(), phase: Phase, scorecard_id: EvaluationIdSchema, source_reference: LocalSourceReferenceSchema, created_at: EvaluationTimestampSchema, provenance: ProvenanceSchema });
...
const DistributionSampleSchema = z.strictObject({ source_scorecard_id: EvaluationIdSchema, source_scorecard_reference: LocalSourceReferenceSchema, run_id: EvaluationIdSchema, trial_id: EvaluationIdSchema, invocation_id: EvaluationIdSchema, value: z.number().finite().nonnegative() });
