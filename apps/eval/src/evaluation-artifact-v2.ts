...
export const V2BenchmarkRunBaseSchema = z.strictObject({
	version: z.literal(2),
...
export const V2BenchmarkRunSchema = V2BenchmarkRunBaseSchema.superRefine((run, context) => {
	...
	for (const [index, declaration] of run.declaration.configurations.entries()) {
		...
		if (Object.values(declaration.config.production_steps).some(({ adapter }) => adapter === "recorded")) {
			context.addIssue({ code: "custom", path: ["declaration", "configurations", index, "config"], message: "artifact version 2 cannot declare a recorded adapter" });
		}
	}
	...
	if (JSON.stringify(run.provenance.output_contracts) !== JSON.stringify(v1OutputContractProvenance())) {
		context.addIssue({ code: "custom", path: ["provenance", "output_contracts"], message: "artifact version 2 requires its exact ordered output-contract representations and hashes" });
	}
...
export type V2BenchmarkRun = z.infer<typeof V2BenchmarkRunSchema>;
