...
import {
	V3_COPYEDIT_SYSTEM_CONSTRAINTS, V3CopyeditPreservationError, V3EditorialOutputError,
	buildV3CopyeditPrompt, parseV3Completion,
...
function version3Finding(error: unknown): z.infer<typeof EvaluationFindingSchema> | undefined {
	if (error instanceof V3EditorialOutputError) {
		return { kind: error.code, production_step: error.productionStep, code: error.code, message: error.message };
	}
	if (error instanceof V3CopyeditPreservationError) {
		return { kind: "preservation", production_step: error.productionStep, code: error.code, message: error.message };
	}
	return undefined;
}

function addVersion3ParseIssue(context: z.RefinementCtx, index: number, message: string): void {
	context.addIssue({ code: "custom", path: ["invocations", index, "parse"], message });
}

export function refineVersion3ParserPromptAndProductRelations(
	...
	for (const trackName of ["main_story", "announcements"] as const) {
		...
		for (const reachedWriter of trial.invocations.filter(({ production_step }) => production_step === writerStep)) {
			if (reachedWriter.request.system !== V3_WRITER_SYSTEM_CONSTRAINTS || reachedWriter.request.user !== expectedWriterUser) {
				context.addIssue({ code: "custom", path: ["invocations", trial.invocations.indexOf(reachedWriter), "request"], message: "every reached writer request must derive exactly from the retained prepared-evidence snapshot under artifact version 3" });
			}
		}
		...
		if (writer?.transport === "succeeded" && writer.parse.state === "succeeded") {
			try {
				writerOutput = trackName === "main_story"
					? V1MainStoryProductSchema.parse(parseV3Completion(writerStep, writer.completion.text))
					: V1AnnouncementsWriterOutputSchema.parse(parseV3Completion(writerStep, writer.completion.text));
				if (!canonicallyEqual(writerOutput, writer.parse.output)) {
					context.addIssue({ code: "custom", path: ["selected_invocation_ids", writerStep], message: "selected writer parse output must equal the artifact version 3 parser result from its retained completion" });
					writerOutput = undefined;
				}
			} catch {
				context.addIssue({ code: "custom", path: ["selected_invocation_ids", writerStep], message: "selected writer completion must parse successfully under artifact version 3" });
			}
		}
		...
		if (copyeditInvocations.length > 0 && writerOutput === undefined) {
			context.addIssue({ code: "custom", path: ["selected_invocation_ids", writerStep], message: "every reached copyedit invocation requires a selected writer validated from its retained completion under artifact version 3" });
			continue;
		}
		...
		for (const copyedit of copyeditInvocations) {
			if (copyedit.request.system !== V3_COPYEDIT_SYSTEM_CONSTRAINTS || copyedit.request.user !== expectedUser) {
				context.addIssue({ code: "custom", path: ["invocations", trial.invocations.indexOf(copyedit), "request"], message: "every reached copyedit request must derive exactly from its selected writer completion under artifact version 3" });
			}
		}
	...
	for (const [index, invocation] of trial.invocations.entries()) {
		...
		try {
			const parsed = parseV3Completion(invocation.production_step, invocation.completion.text, writerOutput);
			if (invocation.parse.state !== "succeeded") addVersion3ParseIssue(context, index, "retained rejection contradicts successful artifact version 3 parsing of the retained completion");
			else if (!canonicallyEqual(parsed, invocation.parse.output)) addVersion3ParseIssue(context, index, "retained parse output must equal the artifact version 3 parser result from the retained completion");
		} catch (error: unknown) {
			const finding = version3Finding(error);
			if (finding === undefined) addVersion3ParseIssue(context, index, "artifact version 3 parser failed without a stable evaluation finding");
			else if (invocation.parse.state !== "rejected" || !canonicallyEqual(invocation.parse.findings, [finding])) addVersion3ParseIssue(context, index, "retained parse rejection must exactly match the artifact version 3 parser finding from the retained completion");
		}
	}
}

export function expectedVersion3FinalProductFindings(
