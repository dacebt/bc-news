...
import { BenchmarkRunSchema, type BenchmarkRun, type V4BenchmarkRun } from "./evaluation-artifact";
...
function assertSnapshots(snapshots: readonly BenchmarkRun[]): void {
	...
	assertProof(snapshots.some((snapshot) => snapshot.version === 4 && snapshot.trials.some((trial) => trial.invocations.some(({ transport }) => transport === "in_flight"))), "in_flight_not_observed", "No pre-transport invocation snapshot was retained");
	assertProof(snapshots.some((snapshot) => snapshot.version === 4 && snapshot.trials.some((trial) => trial.invocations.some((invocation) => invocation.transport === "failed" && invocation.retry_classification.state === "classified"))), "classified_failure_not_observed", "No classified failed invocation snapshot was retained");
}

function assertFinalBenchmark(benchmark: V4BenchmarkRun): void {
...
export async function verifyEvaluationBenchmarkContinuation(temporaryRoot?: string): Promise<void> {
	...
	try {
		...
		assertProof(retained.version === 4 && isDeepStrictEqual(retained, result.benchmark), "retained_artifact_mismatch", "Returned benchmark did not equal its retained artifact");
	} finally {
		await server.close();
