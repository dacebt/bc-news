...
export function assertRecordedReplayAcceptanceDeterminism(first: RunFile, second: RunFile): void {
	...
	if (differences.length > 0) {
		throw new Error(`recorded-replay acceptance is not deterministic: ${differences.join(", ")}`);
	}
}
