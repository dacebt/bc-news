...
import { expectedVersion3FinalProductFindings, refineVersion3ParserPromptAndProductRelations } from "./evaluation-artifact-v3-relations";
...
function refineTrack(trial: EvaluationTrial, trackName: "main_story" | "announcements", evidence: V1PreparedEvidence, context: z.RefinementCtx): void {
	...
	if (track.product !== null && track.terminal_production_step === copyeditStep) {
		...
		try { derived = expectedVersion3FinalProductFindings(trackName, track.product, evidence); }
		catch { context.addIssue({ code: "custom", path: ["tracks", trackName, "product"], message: "retained product must match the frozen artifact version 3 product contract" }); }
		if (Array.isArray(derived)) {
			if (track.lifecycle === "completed" && derived.length > 0) context.addIssue({ code: "custom", path: ["tracks", trackName, "findings"], message: "completed track must have zero frozen version 3 final-product findings" });
			...
			if (track.subject_outcome === "final_product_rejected" && (derived.length === 0 || !canonicallyEqual(track.findings, derived))) context.addIssue({ code: "custom", path: ["tracks", trackName, "findings"], message: "final-product rejection findings must be the nonempty exact frozen version 3 re-derivation" });
		}
	}
}

export function refineVersion3Trial(
	...
	refineVersion3ParserPromptAndProductRelations(trial, evidence, trialContext);
