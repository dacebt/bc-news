...
export async function executeEvaluationTrial(input: {
	...
	function retainedTrial(retainedBenchmark: V5BenchmarkRun): V5EvaluationTrial {
	...
	function enqueueMutation<T>(derive: (current: V5BenchmarkRun) => { readonly next: V5BenchmarkRun; readonly result: T }): Promise<T> {
		const operation = mutationTail.then(async () => {
			if (mutationFailed) throw mutationFailure;
			try {
				const mutation = derive(benchmark);
				const next = V5BenchmarkRunSchema.parse(mutation.next);
	...
	function updateTrial<T>(derive: (current: V5EvaluationTrial) => { readonly next: V5EvaluationTrial; readonly result: T }): Promise<T> {
	...
	async function runMainStoryTrack(): Promise<void> {
		...
		if (mainStory === undefined) {
			await updateTrial((current) => ({ next: { ...current, tracks: { ...current.tracks, main_story: { ...current.tracks.main_story, lifecycle: "rejected", subject_outcome: terminalVersion5TrackOutcome(current, "main_story"), terminal_production_step: mainDraft === undefined ? "main_story_write" : "main_story_copyedit" } } }, result: undefined }));
		} else {
	...
	async function runAnnouncementsTrack(): Promise<void> {
		...
		if (announcements === undefined) {
			await updateTrial((current) => ({ next: { ...current, tracks: { ...current.tracks, announcements: { ...current.tracks.announcements, lifecycle: "rejected", subject_outcome: terminalVersion5TrackOutcome(current, "announcements"), terminal_production_step: announcementsDraft === undefined ? "announcements_write" : "announcements_copyedit" } } }, result: undefined }));
		} else {
