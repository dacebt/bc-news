...
export class V1EditorialOutputError extends Error {
	constructor(readonly productionStep: V1ProductionModelStep, readonly code: "invalid_json" | "contract_mismatch", message: string, options?: ErrorOptions) {
		super(message, options); this.name = "V1EditorialOutputError";
	}
}

export class V1CopyeditPreservationError extends Error {
	constructor(readonly productionStep: CopyeditStep, readonly code: PreservationCode, message: string) {
		super(message); this.name = "V1CopyeditPreservationError";
	}
}
...
function parseJson(step: V1ProductionModelStep, text: string | null): unknown {
	...
	catch (cause) { throw new V1EditorialOutputError(step, "invalid_json", `${step} model output is not valid JSON`, { cause }); }
}
...
function parseSchema<T>(step: V1ProductionModelStep, text: string | null, schema: z.ZodType<T>): T {
	...
	if (!result.success) throw new V1EditorialOutputError(step, "contract_mismatch", `${step} model output does not match its strict contract: ${result.error.message}`);
	return result.data;
}

export function attachV1AnnouncementIds(draft: V1AnnouncementsProduct) {
...
function paragraphCount(text: string): number { return text.replace(/\r\n?|\u0085|\u2028/gu, "\n").replace(/\u2029/gu, "\n\n").split(/\n[^\S\n]*\n/u).length; }
...
function preserve(step: CopyeditStep, fields: readonly (readonly [string, string, string])[]): void {
	for (const [path, before, after] of fields) {
		if (paragraphCount(before) !== paragraphCount(after)) throw new V1CopyeditPreservationError(step, "paragraph_count", `Copyedit changed paragraph count in ${path}`);
		if (!same(quotedSpans(before), quotedSpans(after))) throw new V1CopyeditPreservationError(step, "quoted_span", `Copyedit changed quoted spans or their order in ${path}`);
		if (!same(matches(before, /[+\-\p{Sm}\p{Pd}]?(?:\p{Nd}+(?:[\p{P}\p{Zs}]\p{Nd}+)*|[.,\u066B\u066C]\p{Nd}+)/gu), matches(after, /[+\-\p{Sm}\p{Pd}]?(?:\p{Nd}+(?:[\p{P}\p{Zs}]\p{Nd}+)*|[.,\u066B\u066C]\p{Nd}+)/gu))) throw new V1CopyeditPreservationError(step, "numeric_literal", `Copyedit changed numeric literals or their order in ${path}`);
		if (!same(matches(before, /\*\*[^*\n]+\*\*|(?<!\*)\*(?!\*)[^*\n]+\*(?!\*)|__[^_\n]+__|(?<!_)_(?!_)[^_\n]+_(?!_)/gu), matches(after, /\*\*[^*\n]+\*\*|(?<!\*)\*(?!\*)[^*\n]+\*(?!\*)|__[^_\n]+__|(?<!_)_(?!_)[^_\n]+_(?!_)/gu))) throw new V1CopyeditPreservationError(step, "protected_markdown", `Copyedit changed protected bold or italic spans or their order in ${path}`);
	}
}

export function parseV1Completion(step: V1ProductionModelStep, text: string | null, writerOutput?: V1WriterOutput): Record<string, unknown> {
	...
	if (step === "main_story_copyedit") {
		...
		if ((draft.main_story.image === undefined) !== (product.main_story.image === undefined)) throw new V1CopyeditPreservationError(step, "field_shape", "Copyedit changed the optional main-story image shape");
		if (draft.main_story.image?.url !== product.main_story.image?.url) throw new V1CopyeditPreservationError(step, "protected_value", "Copyedit changed the main-story image URL");
		...
		if (draft.main_story.image !== undefined && product.main_story.image !== undefined && (draft.main_story.image.credit === undefined) !== (product.main_story.image.credit === undefined)) throw new V1CopyeditPreservationError(step, "field_shape", "Copyedit changed the optional main-story image credit shape");
		return product;
	}
	const draft = attachV1AnnouncementIds(V1AnnouncementsWriterOutputSchema.parse(writerOutput));
	const edited = parseSchema(step, text, V1AnnouncementsCopyeditOutputSchema);
	if (edited.announcements.length !== draft.announcements.length) throw new V1CopyeditPreservationError(step, "announcement_count", "Copyedit changed the announcement count");
	for (let index = 0; index < draft.announcements.length; index += 1) {
		...
		if (before.id !== after.id) throw new V1CopyeditPreservationError(step, "announcement_identity", `Copyedit changed or reordered announcement id ${before.id} at index ${index}`);
...
export const V1_COPYEDIT_SYSTEM_CONSTRAINTS = `
[ROLE]
...
function fenceV1Json(label: string, data: unknown): string {
