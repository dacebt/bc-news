...
export async function verifyEvaluationTrialRetention(): Promise<void> {
	...
	try {
		...
		assertProof(writerInterruption.version === 5 && writerInterruption.lifecycle === "running", "writer_interruption_invalid", "Concurrent writer interruption was not a strict running v5 artifact");
		...
		assertProof(interruptedHarnessArtifact.version === 5 && interruptedHarnessArtifact.lifecycle === "running" && interruptedHarnessArtifact.harness_outcome === "pending", "harness_failure_not_running", "Harness failure did not leave a strict running v5 artifact");
