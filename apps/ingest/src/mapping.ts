import { ActiveRegionIdSchema } from "@bc-news/contracts";
...
export function mapMessageToRow(msg: BitJitaMessage): ChatMessageRow | null {
	...
	const parsedRegionId = ActiveRegionIdSchema.safeParse(msg.regionId);
	const channelId = parseDecimalInteger(msg.channelId);
	if (!parsedRegionId.success || channelId === null) {
		return null;
	}
	const regionId = Number(parsedRegionId.data);
