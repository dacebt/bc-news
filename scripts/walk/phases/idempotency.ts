...
async function run(ctx: WalkContext): Promise<void> {
	...
	if (ctx.state.firstModelUsageBody === undefined) {
		throw new Error("idempotency phase requires operator-status model usage evidence");
	}
	const firstModelUsageBody = ctx.state.firstModelUsageBody;
	...
	if (JSON.stringify(status.model_usage) !== firstModelUsageBody) {
