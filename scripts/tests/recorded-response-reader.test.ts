...
void test("the walk reader accepts the committed strict v3 main-story response fixture", async () => {
	...
	assert.equal(response.provider, "synthetic_fixture");
	assert.equal(response.model, "game-reference-main-story-v3");
	assert.match(response.prompt_sha256, /^[0-9a-f]{64}$/u);
	assert.match(response.text, /\[\[GAME_REF_001\]\]/u);
});
...
void test("the walk reader accepts the committed strict v3 announcements response fixture", async () => {
	...
	assert.equal(response.provider, "synthetic_fixture");
	assert.equal(response.model, "game-reference-announcements-v3");
	assert.match(response.prompt_sha256, /^[0-9a-f]{64}$/u);
	assert.match(response.text, /\[\[GAME_REF_002\]\]/u);
});
